...
def main(buffer_days: int = 45):
    universe_path = "universe/stock_kospi200_kosdaq150.csv"
    ...
    frames = []
    for t in tqdm(tickers, desc=f"Updating STOCK from {start}"):
        try:
    ...
    print(f"[OK] STOCK updated: start={start} rows={len(merged)} tickers={merged['ticker'].nunique()} last={merged['date'].max()}")
